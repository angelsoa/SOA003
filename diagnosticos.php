...
$arrayResultados2=[];  
 if(isset($result['metadata']['textfacets'])){
    ...
    for($k = 0; $k < count($result['metadata']['textfacets']); $k++) {
      ...
      $searchCode=preg_match("/cie10_diagnostico/",$path);
      //echo $searchCode.'<br>';
      if($searchCode===1){
...
// echo "</pre>";
    ...
    for($c = 0; $c < count($arrayD00); $c++) {
     for($k = 0; $k < count($result['metadata']['textfacets']); $k++) { 
        ...
        $path=$result['metadata']['textfacets'][$k]['path'][0];
        $searchDiag=preg_match("/descripcion_diagnosticos/",$path);
...
if(isset($result1['metadata']['textfacets'])){
    ...
    for($k = 0; $k < count($result1['metadata']['textfacets']); $k++) {
      ...
      $searchCode1=preg_match("/cie10_diagnostico/",$path1);
      //echo $searchCode.'<br>';
      if($searchCode1===1){
...
// echo "</pre>";
    ...
    for($c = 0; $c < count($arrayD02); $c++) {
      for($k = 0; $k < count($result1['metadata']['textfacets']); $k++){
        ...
        $path1=$result1['metadata']['textfacets'][$k]['path'][0];
        $searchDiag1=preg_match("/descripcion_diagnosticos/",$path1);
        $diagnosticos1=$result1['metadata']['textfacets'][$k]['keyword'];
        $beginDiag1=$result1['metadata']['textfacets'][$k]['begin'];
        // echo $searchDiag1."<br>";
        if($searchDiag1===1 & $beginCode1===$beginDiag1){             $arrayD02[$c][2]=$diagnosticos1;
...
$datetime =  date_default_timezone_set('America/Mexico_City'); 
$fecha=(date ("d-m-Y"));
$archivo="log ".$fecha.".txt";
$ip = $_SERVER['REMOTE_ADDR'];
$hora=("Date: ".date ("d/m/Y - H:i:s")).PHP_EOL;
$text="Text: "."[ ".$text." ]".PHP_EOL.PHP_EOL;
$datos="";
$datos1="";
...
if( count( $arrayResultados2 ) === 0 ){
  ...
  // var_dump(isset($arrayResultados2[$l][$m]));
      if($m%2===1 && isset($arrayResultados2[$l][$m]) ){
        ...
        $datos= $datos."*".$arrayResultados2[$l][$m];
      }

      if($m%2===0 && isset($arrayResultados2[$l][$m]) ){
        $datos1= $datos1."*".$arrayResultados2[$l][$m];
      }
    }
  }

  // var_dump($datos1);
  $arrayCodigos = explode("*", $datos1);
  //var_dump($arrayCodigos);

  // var_dump($datos);
  $arrayDiag = explode("*", $datos);
  //var_dump($arrayDiag);

  $file=fopen($archivo,"a"); 
  fwrite($file,("***********************************".PHP_EOL.PHP_EOL.$hora.$text."Response: ".PHP_EOL));
  for($l = 1; $l<=max(array_keys($arrayCodigos)); $l++) {    
    fwrite($file,($arrayCodigos[$l]." ------- ".$arrayDiag[$l].PHP_EOL)).PHP_EOL;
  } 
  fclose($file);
