...
if( count( $arrayResultados ) === 0 ){
     // echo PHP_EOL."Procedimientos vacios";//empty arrayResultados
  echo "";
}


else{
  for($l = 0; $l <=max(array_keys($arrayResultados)); $l++) {                    
    //echo '<tr id="trTablaSuperior">';
    // for($m = 0; $m <=max(array_keys($arrayResultados[$l])); $m++) {
      // if(empty($arrayResultados[$l][0])){
      //   // echo "<tr><td id='tdDiagnosticos' style='border-top:1px solid #dddddd;border-bottom:1px solid #dddddd;'><a id='linkDiagnosticos'>".$arrayResultados[$l][1].'</a></td></tr>';

      //    echo "*".$arrayResultados[$l][1];
      // }
      // else{
